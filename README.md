...
---
## 7. 실습 예제 & 종합 정리 (Practice & Tips)

NumPy의 모든 기능을 종합하여 실무에서 자주 쓰이는 패턴과 주의할 점 정리.

### 1️⃣ 조건부 값 변경 (`np.where`) - ⭐강력 추천
반복문(`for`)과 `if`문을 대체하는 가장 빠르고 직관적인 방법.
> **문법:** `np.where(조건, 참일_때_값, 거짓일_때_값)`

```python
import numpy as np

# 1. 이진 분류 결과 변환 (Thresholding)
probs = np.array([0.1, 0.6, 0.8, 0.3])
# 0.5 이상이면 1, 아니면 0으로 변환
preds = np.where(probs >= 0.5, 1, 0)  # [0, 1, 1, 0]

# 2. 값 필터링 (ReLU 함수 원리)
vals = np.array([-10, 5, -2, 8])
# 0보다 큰 값은 그대로(vals), 작으면 0으로 치환
relu = np.where(vals > 0, vals, 0)    # [0, 5, 0, 8]
```

### 2️⃣ RMSE 계산과 요소별 연산 (Element-wise)
**헷갈렸던 포인트:** "왜 행렬 곱(`@`, `dot`)을 안 쓰고 굳이 제곱(`**2`) 후 평균(`mean`)을 낼까?"

* **행렬 곱(`@`)**: 차원이 높아지면(2차원 이상) 데이터 간에 **섞임**이 발생하여, 단순 합(Sum of Squares)을 구할 수 없음.
* **요소별 연산(`**`, `+`, `-` 등)**: 차원(Shape)에 상관없이 **각 위치의 원소끼리만** 연산하므로 안전함.
...
# ✅ 정석 방법 (차원 무관, 안전함)
# 오차 -> 제곱(부호 제거) -> 평균(Mean) -> 루트(Root)
rmse = np.sqrt(np.mean((y_pred - y_true)**2))

# ❌ 행렬 곱 사용 시 주의 (1차원 벡터일 때만 작동)
# 2차원(배치 데이터)에서는 데이터끼리 곱해져서 엉뚱한 값이 나옴
# rmse_dot = np.sqrt((error @ error) / N) (비추천)
```

### 3️⃣ 원-핫 인코딩 (One-Hot Encoding) 꿀팁
정수 레이블을 원-핫 벡터로 변환하는 가장 `NumPy`스러운 방법.
> **원리:** 단위 행렬(`np.eye`)에서 **팬시 인덱싱**으로 원하는 행을 쏙쏙 뽑아옴.

```python
labels = np.array([0, 2, 1]) # 정답 클래스
num_classes = 3

# np.eye(3) 행렬에서 labels에 적힌 순서대로 행 추출
...
### 4️⃣ 기타 실무 팁
* **무한대 처리:** `value > 99999` 같은 임의의 숫자 대신 `np.isinf(value)` 사용 권장.
* **타입 변환:** NumPy 타입을 Python `float`로 바꿀 때 `.item()` 사용 (JSON 저장 시 에러 방지).
